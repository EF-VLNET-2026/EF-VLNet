...
class EchoNetStyleVisualizer:
    ...
    def create_enhanced_scatter_plot(self, predictions, targets, filename, phase='test', fontsize=14):
        correlation = np.corrcoef(predictions, targets)[0, 1] if len(predictions) > 1 else 0

        plt.figure(figsize=(8, 6))
        plt.scatter(predictions, targets, s=5, alpha=0.6, label='Data Points')
        plt.xlabel('Predicted EF (%)', fontsize=fontsize)
        plt.ylabel('Actual EF (%)', fontsize=fontsize)
        plt.title(f'Predicted vs Actual EF (r={correlation:.3f})', fontsize=fontsize)
        ...
        plt.close()
        logger.info(f"Enhanced scatter plot saved: {plot_path}")

        return correlation

    def create_bland_altman_plot(self, predictions, targets, filename, fontsize=14):
        ...
        if len(predictions) == 0:
            logger.error("Error: No valid data points for Bland-Altman plot")
            return None
        ...
        logger.info(f"Bland-Altman statistics:")
        logger.info(f"  Mean difference: {md:.4f}")
        logger.info(f"  Std dev of difference: {sd:.4f}")
        logger.info(f"  95% Limits of Agreement: [{md - 1.96*sd:.4f}, {md + 1.96*sd:.4f}]")

        plt.figure(figsize=(8, 6))
        plt.scatter(mean, diff, s=5, alpha=0.6)

        plt.axhline(md, color='blue', linestyle='-', linewidth=2,
                    label=f'Mean difference ({md:.4f})')
        ...
        plt.xlabel('Mean of Predicted EF and Actual EF (%)', fontsize=fontsize)
        plt.ylabel('Difference (Predicted EF - Actual EF) (%)', fontsize=fontsize)
        plt.title('Bland-Altman Plot', fontsize=fontsize)
        ...
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.close()
        logger.info(f"Bland-Altman plot saved: {plot_path}")

        return {
            'mean_difference': float(md),
            'std_difference': float(sd),
            'upper_loa': float(upper_limit),
            'lower_loa': float(lower_limit)
        }
...
class OptimizedFourLayerDataset(Dataset):
    ...
    def __init__(
        ...
        logger.info(f"\nDataset loading complete: {len(self.valid_samples)} samples")
    ...
    def _preprocess_temporal_df(self) -> Dict:
        ...
        logger.info("\nPreprocessing temporal data dictionary...")
        temporal_dict = {}
        ...
        for chamber in self.CHAMBERS:
            for phase in self.PHASES:
                ...
                if self.model_type == 'r3d':
                    base_path = f"{chamber.lower()}_r3d_{phase_dir}_edv_esv"
                else:
                ...
                if feature_file and feature_file.exists():
                    try:
                        ...
                        if feature_sum < 1e-6:
                            logger.warning(f"  {chamber}_{phase}: Features all zero! (sum={feature_sum:.2e})")
                        ...
                        logger.warning(f"  {chamber}_{phase}: Loading failed - {e}")
                else:
                    logger.warning(f"  {chamber}_{phase}: File not found")
            ...
            if total > 0:
                ...
                if 'all_zero' in stats:
                    ...
                    if all_zero > 0:
                        ...
                        logger.error(f"    Warning - Zero features: {all_zero} ({zero_rate:.1f}%)")
...
class ConservativeTrainer:
    def __init__(
        ...
        max_epochs: int = 100,
        patience: int = 16,
        save_dir: str = "/path/to/your/own/checkpoints",
        warmup_epochs: int = 3
    ):
    ...
    def test(self, test_loader: DataLoader) -> Dict:
        ...
        if pred_std < 1e-6:
            logger.error(f"Error: Prediction std dev near zero: {pred_std:.10f}")
        ...
        self.visualizer.create_scatter_plot(
            ...
            "Test Set Performance", "test_scatter_echonet.pdf", phase='test'
        )

        self.visualizer.create_enhanced_scatter_plot(
            all_preds, all_targets,
            "test_scatter_enhanced.png", phase='test'
        )

        bland_altman_stats = self.visualizer.create_bland_altman_plot(
            all_preds, all_targets,
        ...
        results_df = pd.DataFrame({
            ...
            'Predicted_EF': all_preds,
            'Actual_EF': all_targets,
            'Error': errors,
            'Abs_Error': abs_errors
        })
        results_csv_path = self.save_dir / "test_detailed_predictions.csv"
