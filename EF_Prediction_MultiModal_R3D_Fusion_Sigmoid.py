...
class OptimizedFourLayerDataset(Dataset):
    ...
    def __init__(
        ...
        logger.info("=" * 80)
        logger.info(f"Initializing dataset: split={self.split}")
        logger.info("=" * 80)
        ...
        self.loading_stats = {
            ...
            'r3d': {'total': 0, 'loaded': 0, 'missing': 0, 'all_zero': 0}
        }
        ...
        self.r3d_features = self._load_r3d_features() if use_r3d else {}
    ...
    def _preprocess_temporal_df(self) -> Dict:
        ...
        logger.info(f"  Processed: {len(temporal_dict)} videos")
    ...
    def _load_r3d_features(self) -> Dict[str, torch.Tensor]:
        r3d_features = {}

        logger.info("\nLoading R3D features...")
        ...
        for chamber in self.CHAMBERS:
            for phase in self.PHASES:
                ...
                for epoch_num in [0, 1, 2, 3, 4, 5]:
                    candidate = self.base_dir / f"{chamber.lower()}_r3d_{phase_dir}" / "features" / f"features_{self.split}_epoch_{epoch_num}.npz"
                    if candidate.exists():
                        feature_file = candidate
                        logger.info(f"  {chamber}_{phase}: Found epoch_{epoch_num} file")
                        break

                if feature_file and feature_file.exists():
                    try:
                        data = np.load(feature_file, allow_pickle=True)
                        self.loading_stats['r3d']['total'] += len(data['filenames'])
                        ...
                        if feature_sum < 1e-6:
                            ...
                            self.loading_stats['r3d']['all_zero'] += len(data['filenames'])
                        else:
                        ...
                        for video_id, feat in zip(data['filenames'], data['features']):
                            ...
                            if video_id not in r3d_features:
                                r3d_features[video_id] = {}
                            r3d_features[video_id][f"{chamber}_{phase}"] = torch.from_numpy(feat).float()
                            self.loading_stats['r3d']['loaded'] += 1
    ...
    def _print_loading_stats(self):
        ...
        for feature_type, stats in self.loading_stats.items():
            ...
            if total > 0:
                ...
                if feature_type == 'r3d' and 'all_zero' in stats:
                    ...
                    if all_zero > 0:
                        ...
                        logger.error(f"    Warning: R3D feature extraction might have failed, please check feature files!")
    ...
    def __getitem__(self, idx):
        ...
        r3d_feat = None
        if self.use_r3d and video_id in self.r3d_features:
            r3d_list = []
            for chamber in self.CHAMBERS:
                for phase in self.PHASES:
                    key = f"{chamber}_{phase}"
                    if key in self.r3d_features[video_id]:
                        r3d_list.append(self.r3d_features[video_id][key])
                    else:
                        r3d_list.append(torch.zeros(128))
            r3d_feat = torch.stack(r3d_list, dim=0)
...
def collate_fn(batch):
    ...
    r3d_feats = torch.stack([
        item['r3d_feat'] if item['r3d_feat'] is not None else torch.zeros(8, 128)
        for item in batch
    ])
...
class ImprovedFourLayerFusionModel(nn.Module):
    def __init__(
        ...
        phase_dim: int = 8,
        temporal_dim: int = 16,
        r3d_chamber_dim: int = 128,
        num_chambers: int = 4,
        num_r3d_features: int = 8,
        hidden_dim: int = 256,
        lstm_hidden: int = 256,
        dropout: float = 0.25
    ):
        ...
        self.r3d_encoder = nn.Sequential(
            nn.Linear(r3d_chamber_dim, hidden_dim // 4),
            nn.BatchNorm1d(hidden_dim // 4),
            nn.ReLU(),
            nn.Dropout(dropout)
        )
        ...
        fusion_dim = (
            ...
            (hidden_dim // 4) * num_r3d_features
        )

        self.attention_fusion = AttentionFusion(fusion_dim, num_heads=8)
    ...
    def forward(self, ai_feats, phase_feats, temporal_feats, r3d_feats):
        ...
        r3d_feats_reshaped = r3d_feats.view(batch_size * 8, 128)
        r3d_encoded = self.r3d_encoder(r3d_feats_reshaped)
        r3d_encoded = r3d_encoded.view(batch_size, -1)
...
class Trainer:
    def __init__(
        ...
        self.error_log_path = self.save_dir / "error.log"
    ...
    def _initialize_csv_log(self):
        with open(self.log_path, 'w') as f:
            f.write("epoch,phase,loss,r2_score,correlation,mae,rmse,elapsed_time,samples,"
                    "memory_allocated_mb,memory_reserved_mb,batch_size,lr,"
                    "min_prediction,max_prediction,negative_preds,over100_preds\n")
    ...
    def _adjust_learning_rate_warmup(self, epoch):
        if epoch <= self.warmup_epochs:
            warmup_factor = 0.7 + 0.3 * (epoch / self.warmup_epochs)
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(pbar):
            try:
                ...
                r3d_feats = batch['r3d_feats'].to(self.device)
                targets = batch['efs'].to(self.device)

                self.optimizer.zero_grad()
                predictions = self.model(ai_feats, phase_feats, temporal_feats, r3d_feats)
                ...
                error_msg = f"Training batch {batch_idx} failed: {str(e)}"
        ...
        memory_allocated = torch.cuda.max_memory_allocated() / 1024**2 if torch.cuda.is_available() else 0
        memory_reserved = torch.cuda.max_memory_reserved() / 1024**2 if torch.cuda.is_available() else 0
        min_pred = np.min(all_preds) if all_preds else 0
        max_pred = np.max(all_preds) if all_preds else 0
        negative_preds = sum(1 for p in all_preds if p < 0)
        over100_preds = sum(1 for p in all_preds if p > 100)

        with open(self.log_path, 'a') as f:
            f.write(f"{epoch},train,{avg_loss:.6f},{r2:.6f},0.0,{mae:.6f},{rmse:.6f},"
                   f"{elapsed_time:.2f},{len(all_preds)},{memory_allocated:.2f},{memory_reserved:.2f},"
                   f"{self.train_loader.batch_size},{self.optimizer.param_groups[0]['lr']:.8f},"
                   f"{min_pred:.2f},{max_pred:.2f},{negative_preds},{over100_preds}\n")

        if negative_preds > 0 or over100_preds > 0:
            logger.warning(f"Warning: Train set: {negative_preds} negative predictions, {over100_preds} predictions > 100%")
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(self.val_loader, desc="Validation")):
                try:
                    ...
                    error_msg = f"Validation batch {batch_idx} failed: {str(e)}"
                    self._log_error(error_msg)
                    logger.error(error_msg)
                    continue
        ...
        with open(self.log_path, 'a') as f:
            f.write(f"{epoch},val,{avg_loss:.6f},{r2:.6f},{correlation:.6f},{mae:.6f},{rmse:.6f},"
                   f"{elapsed_time:.2f},{len(all_preds)},{memory_allocated:.2f},{memory_reserved:.2f},"
                   f"{self.val_loader.batch_size},{self.optimizer.param_groups[0]['lr']:.8f},"
                   f"{min_pred:.2f},{max_pred:.2f},{negative_preds},{over100_preds}\n")

        if negative_preds > 0 or over100_preds > 0:
            logger.warning(f"Warning: Validation set: {negative_preds} negative predictions, {over100_preds} predictions > 100%")
            ...
            for batch_idx, batch in enumerate(tqdm(test_loader, desc="Testing")):
                try:
                    ...
                    if debug_first_batch:
                        logger.info("\n" + "=" * 80)
                        logger.info("First batch detailed diagnosis:")
                        logger.info("=" * 80)
                        logger.info(f"\nR3D feature stats:")
                        logger.info(f"  shape={r3d_feats.shape}")
                        logger.info(f"  mean={r3d_feats.mean():.6f}, std={r3d_feats.std():.6f}")
                        logger.info(f"  min={r3d_feats.min():.6f}, max={r3d_feats.max():.6f}")

                        zero_ratio = (r3d_feats.abs() < 1e-8).float().mean()
                        logger.info(f"  Zero ratio: {zero_ratio:.2%}")
                        if zero_ratio > 0.99:
                            logger.error(f"  Error: R3D features almost all zero!")
                            logger.error(f"  Error: Will cause model to output identical predictions!")
                    ...
                    if batch_idx == 0:
                        ...
                        logger.info(f"  Predictions (top 10): {predictions[:10].cpu().numpy()}")
                        logger.info(f"  Unique values: {len(torch.unique(predictions))}")
                        if len(torch.unique(predictions)) == 1:
                            logger.error("  Error: All predictions are identical!")
                    ...
                    error_msg = f"Test batch {batch_idx} failed: {str(e)}"
                    self._log_error(error_msg)
                    logger.error(error_msg)
                    continue
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.max_epochs + 1):
            ...
            try:
                ...
                if val_r2 > self.best_val_r2:
                    ...
                    self.best_checkpoint_path = self.save_dir / f"best_model_r2_{val_r2:.4f}.pt"
                    torch.save(checkpoint, self.best_checkpoint_path)
                    logger.info(f"  Saved best model: {self.best_checkpoint_path.name}")
        ...
        logger.info("\n" + "=" * 80)
        logger.info(f"Training completed! Best R²: {self.best_val_r2:.4f}")
        logger.info("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(description="EF Prediction Training Script")
    ...
    if args.test_only:
        ...
        if not feature_scalers:
            temp_train_dataset = OptimizedFourLayerDataset(
                ...
                use_temporal=True, use_r3d=False,
        ...
        test_dataset = OptimizedFourLayerDataset(
            ...
            use_temporal=True, use_r3d=True,
        ...
        model = ImprovedFourLayerFusionModel()
    ...
    trainer = Trainer(
        ...
        device=device, ef_normalizer=ef_normalizer,
        feature_scalers=feature_scalers,
        learning_rate=args.lr, max_epochs=args.epochs,
