...
class OptimizedFourLayerDataset(Dataset):
    ...
    def __init__(
        ...
        self.use_r2plus1d = use_r2plus1d
        ...
        self.r2plus1d_features = self._load_video_features() if use_r2plus1d else {}
        ...
        for chamber in self.CHAMBERS:
            for phase in self.PHASES:
                ...
                for epoch_num in [0, 1, 2, 3, 4, 5]:
                    candidate = self.base_dir / f"{chamber.lower()}_{self.model_type}_{phase_dir}_video_only" / "features" / f"features_{self.split}_epoch_{epoch_num}.npz"

                    if candidate.exists():
                        feature_file = candidate
                        logger.info(f"  {chamber}_{phase}: Found epoch_{epoch_num} file")
                        logger.info(f"    Path: {candidate}")
                        break
                ...
                if feature_file and feature_file.exists():
                    ...
                    logger.warning(f"    Expected path format: {self.base_dir}/{chamber.lower()}_{self.model_type}_{phase_dir}_video_only/features/features_{self.split}_epoch_*.npz")
            ...
            if total > 0:
                ...
                if 'all_zero' in stats:
                    ...
                    if all_zero > 0:
                        ...
                        logger.error(f"    Warning: {self.model_type.upper()} feature extraction might have failed, please check feature files!")
    ...
    def __getitem__(self, idx):
        ...
        video_feat = None
        if self.use_r2plus1d and video_id in self.r2plus1d_features:
            video_list = []
            for chamber in self.CHAMBERS:
                for phase in self.PHASES:
                    key = f"{chamber}_{phase}"
                    if key in self.r2plus1d_features[video_id]:
                        video_list.append(self.r2plus1d_features[video_id][key])
                    else:
            ...
            for batch_idx, batch in enumerate(tqdm(test_loader, desc="Testing")):
                try:
                    ...
                    if debug_first_batch:
                        ...
                        logger.info("=" * 80)
                        logger.info(f"\nVideo feature stats:")
                        logger.info(f"  shape={video_feats.shape}")
                        logger.info(f"  mean={video_feats.mean():.6f}, std={video_feats.std():.6f}")
                        logger.info(f"  min={video_feats.min():.6f}, max={video_feats.max():.6f}")

                        zero_ratio = (video_feats.abs() < 1e-8).float().mean()
                        logger.info(f"  Zero ratio: {zero_ratio:.2%}")
                        if zero_ratio > 0.99:
                            logger.error(f"  Error: Video features almost all zero!")
                            logger.error(f"  Error: This will cause model to output identical predictions!")
...
def main():
    parser = argparse.ArgumentParser(description="EF Prediction Training Script (R2Plus1D version)")
    parser.add_argument('--base_dir', type=str, default='/path/to/your/own/data/EchoNet-Dynamic')
    parser.add_argument('--batch_size', type=int, default=32)
    parser.add_argument('--lr', type=float, default=1e-3)
    parser.add_argument('--epochs', type=int, default=100)
    parser.add_argument('--save_dir', type=str, default='/path/to/your/own/checkpoints_r2plus1d')
    ...
    if args.test_only:
        ...
        if not feature_scalers:
            temp_train_dataset = OptimizedFourLayerDataset(
                ...
                use_temporal=True, use_r2plus1d=False,
        ...
        test_dataset = OptimizedFourLayerDataset(
            ...
            use_temporal=True, use_r2plus1d=True,
            model_type=args.model_type,
